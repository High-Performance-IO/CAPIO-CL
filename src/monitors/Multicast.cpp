...
capiocl::monitor::MulticastMonitor::MulticastMonitor(
    ...
    START_LOG(calf_current_tid(), "call()");
    config.getParameter("capiocl.monitor.mcast.commit.ip", &MULTICAST_COMMIT_ADDR,
                        configuration::defaults::DEFAULT_MONITOR_MCAST_IP.v);

    config.getParameter("capiocl.monitor.mcast.commit.port", &MULTICAST_COMMIT_PORT,
                        std::stoi(configuration::defaults::DEFAULT_MONITOR_MCAST_PORT.v));

    config.getParameter("capiocl.monitor.mcast.homenode.ip", &MULTICAST_HOME_NODE_ADDR,
                        configuration::defaults::DEFAULT_MONITOR_HOMENODE_IP.v);

    config.getParameter("capiocl.monitor.mcast.homenode.port", &MULTICAST_HOME_NODE_PORT,
                        std::stoi(configuration::defaults::DEFAULT_MONITOR_HOMENODE_PORT.v));

    config.getParameter("capiocl.monitor.mcast.delay_ms", &MULTICAST_DELAY_MILLIS,
